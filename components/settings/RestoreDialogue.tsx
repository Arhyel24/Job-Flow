  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      if (result && result.appData) {
        ...
        const existingJobs: JobApplication[] = existingJobsRaw
          ? JSON.parse(existingJobsRaw)
          : [];
        ...
        await Promise.all([
          ...
          AsyncStorage.setItem(
            LAST_SYNC_KEY,
            result.appData.lastSync || new Date().toISOString()
          ),
          AsyncStorage.setItem(
            PENDING_CHANGES_KEY,
            JSON.stringify(result.appData.pendingChanges || [])
          ),
        ]);
        ...
        Toast.show({
          ...
          text2: `Restored backup from ${formatDate(
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      {loading ? (
        ...
        <>
          <Text
            variant="h3"
            style={{ color: theme.text.primary, marginBottom: 20 }}
          >
            Sign In Required
          </Text>
          <Text
        ...
        <>
          ...
          <Text style={[styles.warning, { color: theme.warning }]}>
            Warning: Restoring will overwrite your current jobs, sync status,
            and pending changes
          </Text>
